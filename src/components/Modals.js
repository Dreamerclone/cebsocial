...
export const ShopModal = ({ viewingShop, user, onClose, onMessage, onSave, t }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[200] flex items-center justify-center p-4" onClick={onClose}>
        <div className="surface-card w-full max-w-2xl rounded-[3rem] overflow-hidden shadow-2xl animate-slide-down flex flex-col md:flex-row" onClick={e => e.stopPropagation()}>
            ...
            <div className="w-full md:w-1/2 p-8 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <span className="text-[10px] font-black uppercase text-black dark:text-white bg-gray-100 dark:bg-slate-800 px-3 py-1 rounded-lg">{viewingShop.category}</span>
                        ...
                        <p className="text-3xl font-black text-black dark:text-white mt-2 italic">{viewingShop.price}</p>
                    </div>
                ...
                <div className="space-y-4 flex-1">
                    ...
                    <div className="flex items-center space-x-4 p-4 border border-gray-100 dark:border-slate-800 rounded-3xl">
                        <div className="w-12 h-12 rounded-2xl bg-black dark:bg-white flex items-center justify-center text-white dark:text-black font-black overflow-hidden">{viewingShop.seller?.avatar_url ? <img src={viewingShop.seller.avatar_url} className="w-full h-full object-cover" /> : viewingShop.author?.[0]}</div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-8">
                    {viewingShop.user_id !== user.id && (
                        <button onClick={() => onMessage(viewingShop.user_id)} className="bg-black dark:bg-white text-white dark:text-black py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:opacity-80 shadow-xl">Message Seller</button>
                    )}
                    <button onClick={() => onSave(viewingShop.id)} className={`py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${viewingShop.isSaved ? 'bg-black text-white dark:bg-white dark:text-black shadow-lg shadow-gray-200' : 'bg-gray-100 dark:bg-slate-800 text-gray-600'} ${viewingShop.user_id === user.id ? 'col-span-2' : ''}`}>{viewingShop.isSaved ? 'Saved' : 'Save for Later'}</button>
                </div>
            </div>
        </div>
    </div>
...
export const NeighborModal = ({ viewingNeighbor, user, onClose, onMessage, onViewProfile }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[250] flex items-center justify-center p-4" onClick={onClose}>
        <div className="surface-card w-full max-w-sm rounded-[3rem] overflow-hidden shadow-2xl animate-slide-down border border-gray-100 dark:border-slate-800" onClick={e => e.stopPropagation()}>
            <div className="h-24 bg-primary-600 relative"><button onClick={onClose} className="absolute top-4 right-4 text-white/50 hover:text-white"><X size={20} /></button></div>
            <div className="px-8 pb-8 text-center -mt-12">
                <div className="w-24 h-24 rounded-[2rem] surface-card p-2 mx-auto shadow-xl"><div className="w-full h-full rounded-[1.5rem] bg-primary-600 overflow-hidden flex items-center justify-center text-white font-black text-2xl">{viewingNeighbor.avatar_url ? <img src={viewingNeighbor.avatar_url} className="w-full h-full object-cover" /> : (viewingNeighbor.full_name?.[0] || '?')}</div></div>
                ...
                <div className="flex items-center justify-center space-x-2 mt-2">
                    <span className="text-[10px] font-black uppercase text-black dark:text-white bg-gray-100 dark:bg-slate-800 px-3 py-1 rounded-lg"><MapPin size={10} className="inline mr-1" /> {viewingNeighbor.neighborhood}</span>
                    <span className="text-[10px] font-black uppercase text-black dark:text-white bg-gray-100 dark:bg-slate-800 px-3 py-1 rounded-lg"><Zap size={10} className="inline mr-1" /> {viewingNeighbor.karma} Karma</span>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 mt-8">
                    <button
                      ...
                      className="bg-black dark:bg-white text-white dark:text-black py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2 shadow-xl"
                    >
...
export const EditItemModal = ({ editingItem, setEditingItem, onSave, isSubmitting }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[260] flex items-center justify-center p-4" onClick={() => setEditingItem(null)}>
        <div className="surface-card w-full max-w-xl rounded-[3rem] overflow-hidden shadow-2xl animate-slide-down border border-gray-100 p-10" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-6">
                ...
                <button disabled={isSubmitting} onClick={onSave} className="w-full bg-black dark:bg-white text-white dark:text-black py-4 rounded-3xl font-black text-xs uppercase shadow-xl flex items-center justify-center space-x-2">
...
export const MemberModal = ({ showMemberModal, setShowMemberModal, members, setViewingNeighbor }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[300] flex items-center justify-center p-4" onClick={() => setShowMemberModal(false)}>
        <div className="surface-card w-full max-w-md rounded-[3rem] overflow-hidden shadow-2xl animate-slide-down border border-gray-100" onClick={e => e.stopPropagation()}>
            ...
            <div className="max-h-[400px] overflow-y-auto p-4 space-y-2">
                {members.map(member => (
                    <div key={member.id} onClick={() => { setViewingNeighbor(member); setShowMemberModal(false); }} className="flex items-center justify-between p-4 rounded-[2rem] hover:bg-gray-100 dark:hover:bg-slate-800 cursor-pointer group transition-colors">
                        <div className="flex items-center space-x-4">
                            <div className="w-12 h-12 rounded-2xl bg-black dark:bg-white flex items-center justify-center text-white dark:text-black font-black overflow-hidden">{member.avatar_url ? <img src={member.avatar_url} className="w-full h-full object-cover" /> : member.full_name?.[0]}</div>
                        ...
                        <div className="flex items-center space-x-2 text-black dark:text-white bg-gray-100 dark:bg-slate-800 px-3 py-1 rounded-xl"><Zap size={10} fill="currentColor" /><span className="text-[10px] font-black">{member.karma}</span></div>
                    </div>
                ))}
            </div>
        </div>
